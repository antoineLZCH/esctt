...
- **Runtimes et déploiements — fait puis conséquence.** Les exigences WordPress reposent sur un serveur PHP, une base de données et un serveur HTTP avec HTTPS ([S4]). Dans le scénario où l’application reste distincte, elle ajoute son propre runtime et son propre déploiement ; la lecture de l’API ne fusionne pas ces processus ([S1]). Le nombre total dépend toutefois du runtime de cette application, encore inconnu.
...
- **Runtimes et déploiements — fait puis scénario.** L’adaptateur serveur Inertia s’installe dans le framework serveur ([S13]) et le guide de déploiement Laravel documente le serveur de production, l’optimisation et une route de santé ([S14]). Le site demande donc un runtime applicatif Laravel ; une application séparée ajoute un déploiement et un runtime serveur si elle en possède un. Un backend Laravel commun pourrait réduire le nombre de processus déployés, mais ferait partager disponibilité, mises à jour, sécurité et calendrier de livraison : c’est un scénario de couplage, pas une décision.
...
- **Runtimes et déploiements — fait puis scénario.** Le déploiement Adonis compile l’application avec `node ace build`, copie les actifs Vite dans la sortie et exécute ensuite le serveur Node de production ([S19]). Le site a donc un runtime Node et une chaîne de build ; une application séparée ajoute son déploiement et son runtime serveur si elle en possède un. Un serveur Adonis commun peut réduire le nombre de déploiements backend, au prix du même partage de disponibilité, sécurité et versions que dans le scénario Laravel.
...
1. **À côté de WordPress.** Même si l’application ne lit que le contenu publié, le runtime CMS, sa base, ses sauvegardes, ses mises à jour et sa supervision restent à exploiter ; l’application ajoute sa propre chaîne. Réutiliser l’API évite de recopier le contenu, pas les responsabilités d’exécution. Cette conclusion est une inférence fondée sur l’API et les exigences WordPress ([S1], [S4]–[S6]).
2. **À côté d’Astro statique.** Le frontend n’ajoute pas de processus serveur Astro, mais il ajoute un build, une publication et un hébergement de fichiers ; l’API éditoriale doit être exploitée en parallèle. Si cette API est un CMS WordPress, la première topologie s’applique aussi. La future application reste une chaîne distincte ; le nombre exact dépend de son architecture ([S7]–[S10]).
3. **Même backend Laravel ou Adonis, responsabilités séparées.** C’est une possibilité technique conditionnelle : elle peut réduire les déploiements backend, mais elle partage les versions, les incidents, la disponibilité, les secrets et les contrôles de sécurité. Séparer les modules logiquement ne supprime pas ce couplage de runtime. Deux backends séparés restaurent l’indépendance mais ne donnent plus l’économie de processus. Ces compromis sont déduits des architectures Inertia et des guides de déploiement ([S12]–[S19]), sans concevoir les modules de la future application.
